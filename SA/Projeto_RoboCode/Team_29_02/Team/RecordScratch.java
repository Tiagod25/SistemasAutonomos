...
public class RecordScratch extends TeamRobot {
    /*
    ...
    */
    ...
    private void updateDisparo() {
        ...
        double relativeBearing = headOnBearing - getHeadingRadians();

        // calcular posição no fim de rodar a arma
        double timeToAim = Utils.normalAbsoluteAngle(relativeBearing) / (20 * Math.PI / 180); // 20 graus por turno
        Vector2D velocity = Vector2D.fromPolarCoords(Math.PI / 2 - getHeadingRadians(), getVelocity());
        Vector2D distanceAiming = velocity.scale(this.rotation * timeToAim);
        ...
        relativeBearing = headOnBearing - getHeadingRadians();

        double linearBearing = headOnBearing + alvo.getFactor() * Math.asin(speed / bulletSpeed * Math.sin(relativeBearing));
    ...
    public void onBulletHit(BulletHitEvent event) {
        this.shotsHit++;
        System.out.println("firing accuracy: " + ((double)this.shotsHit) / this.shotsFired);;
    }
    ...
    public void onHitRobot(HitRobotEvent e){
        ...
        if (flag == 0) {
			double change = (e.getBearing()) % 360;
        	if (change >= -180 && change <= 180) {
            	turnRight(change);
        	} else if (change < -180) {
